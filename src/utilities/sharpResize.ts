...
const imgResize = (location: string): string[] => {
  ...
  const outFolder = path.join('build', 'exported-images');
  let resizedFile = '';
  let outputFile = '';
  ...
  if (imgCache.get(nameOnly)) {
    const imageFiles: string[] = imgCache.get(nameOnly) as string[];
    ...
    try {
      ...
      const htmlImages = path.join('exported-images', nameOnly, resizedFile);
      imgSizes.push(htmlImages);
      sharp(location).resize({ width: 150, height: 200 }).toFile(outputFile);
    } catch (error) {
